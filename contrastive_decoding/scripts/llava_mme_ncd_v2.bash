...
if [[ $neg == false ]]; then
    ...
    experiment=${model_name}-ncd_v2-t${temperature}-a${cd_alpha}-b${cd_beta}-seed${seed}
else
    neg_question_file=llava_eval/MME/llava_mme_gt.jsonl
    question_file=llava_eval/MME/llava_mme_neg.jsonl
    experiment=NEG-${model_name}-ncd_v2-t${temperature}-a${cd_alpha}-b${cd_beta}-seed${seed}
fi
...
python contrastive_decoding/eval/llava_mme_ncd_v2.py \
