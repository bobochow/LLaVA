...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return input_ids_list[0], image_tensor, image_tensor_cd, image.size,input_ids_list[1]
...
def collate_fn(batch):
    input_ids, image_tensors, image_tensor_cd, image_sizes, input_ids_neg = zip(*batch)
    input_ids_batch=[]
    
    input_ids = torch.stack(input_ids, dim=0)
    input_ids_batch.append(input_ids)
    input_ids_neg = torch.stack(input_ids_neg, dim=0)
    input_ids_batch.append(input_ids_neg)
    
    # input_ids = torch.stack(input_ids, dim=0)
    image_tensors = torch.stack(image_tensors, dim=0)
    image_tensor_cd = torch.stack(image_tensor_cd, dim=0)
    return input_ids_batch, image_tensors, image_tensor_cd, image_sizes
...
def eval_model(args):
    ...
    scores=[]
    for (input_ids_list, image_tensor, image_tensor_cd, image_sizes) in tqdm(data_loader, desc="Evaluating:  "):
